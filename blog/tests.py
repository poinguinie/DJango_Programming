...
class TestView(TestCase):
    def setUp(self):
        self.client = Client()
        self.user_Jack = User.objects.create_user(username='jack', password='jack1355!')
        self.user_John = User.objects.create_user(username='john', password='john1355!')

        self.user_John.is_staff = True
        self.user_John.save()
    ...
    def test_category_page(self):
        soup = self.connectSoup(self.category_programming.get_absolute_url())
    ...
    def test_tag_page(self):
        soup = self.connectSoup(self.tag_hello.get_absolute_url())
    ...
    def test_create_post(self):
        ...
        # staff가 아닌 Jack이 로그인을 한다.
        self.client.login(username='jack', password='jack1355!')
        response = self.client.get('/blog/create_post/')
        self.assertNotEqual(response.status_code, 200)

        # staff인 Jogn으로 로그인을 한다.
        self.client.login(username='john', password='john1355!')
        soup = self.connectSoup('/blog/create_post/')
        ...
        self.assertEqual(last_post.author.username, 'john')
    ...
    def test_update_post(self):
        ...
        # 로그인은 했지만 작성자가 아닌 경우
        self.assertNotEqual(self.post_003.author, self.user_Jack)
        self.client.login(
            username=self.user_Jack.username,
            password='jack1355!'
        )
        response = self.client.get(update_post_url)
        ...
        soup = self.connectSoup(update_post_url)
        ...
        response = self.client.post(
            update_post_url,
            {
                ...
                'content': '안녕? 만나서 반가워! 이 포스트는 수정됐어!',
        ...
        self.assertIn('안녕? 만나서 반가워! 이 포스트는 수정됐어!', main_area.text)
    ...
    def test_post_list(self):
        ...
        self.assertIn(self.user_John.username.upper(), main_area.text)
        self.assertIn(self.user_Jack.username.upper(), main_area.text)
    ...
    def test_post_detail(self):
        ...
        soup = self.connectSoup(self.post_001.get_absolute_url())
        ...
        self.assertIn(self.user_Jack.username.upper(), post_area.text)
    ...
    def test_comment_form(self):
        ...
        self.assertIn('john', new_comment_div.text)
        self.assertIn('새 댓글입니다.', new_comment_div.text)
    ...
    def test_comment_update(self):
        ...
        self.assertIn('존의 댓글을 수정합니다.', comment_001_div.text)
        self.assertIn('Updated: ', comment_001_div.text)

    def test_delete_comment(self):
        comment_by_jack = Comment.objects.create(
            post=self.post_001,
            author=self.user_Jack,
            content='잭의 댓글입니다.',
        )
        ...
        self.assertIn(self.post_001.title, soup.title.text)
        comment_area = soup.find('div', id='comment-area')
        self.assertNotIn('잭의 댓글입니다.', comment_area.text)
